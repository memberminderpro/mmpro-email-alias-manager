...
class MMPRO_Email_Alias_Manager
{
    ...
    public function sanitize_aliases_data($input)
    {
        ...
        foreach ($input as $alias_id => $data) {
            if (empty($data['address']) || empty($data['destinations']))
                continue;
        ...
        <div class="wrap mmpro-admin-wrapper">
            ...
            <?php if ($this->has_data_persistence_issue()): ?>
                <div class="data-persistence-warning">
                    ...
                    <p><?php _e('We\'ve detected that your email aliases data is being lost between page loads. This could be caused by:', 'mmpro-email-aliases'); ?>
                    </p>
                    ...
                    <p>
                        <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mmpro-email-aliases&action=recover_data'), 'mmpro_recover_data'); ?>"
            ...
            <div class="mmpro-admin-columns">
                ...
                <div class="mmpro-main-column">
                    ...
                    <div class="mmpro-card">
                        ...
                        <p><?php _e('Create and manage email aliases that will forward to one or more destination addresses.', 'mmpro-email-aliases'); ?>
                        </p>
                        <p><?php _e('Each alias will be available through the API for use with Cloudflare Email Workers.', 'mmpro-email-aliases'); ?>
                        </p>

                        <div class="mmpro-button-row">
                            <button type="button" id="add-alias-top"
                                class="button"><?php _e('Add New Alias', 'mmpro-email-aliases'); ?></button>
                            <button type="submit" form="mmpro-aliases-form" name="submit_aliases"
                                class="button button-primary"><?php _e('Save All Aliases', 'mmpro-email-aliases'); ?></button>
                        </div>
                    </div>
                    ...
                    <form method="post" action="" id="mmpro-aliases-form">
                        ...
                        <div class="mmpro-button-row" style="margin-top: 20px;">
                            <button type="button" id="add-alias-bottom"
                                class="button"><?php _e('Add New Alias', 'mmpro-email-aliases'); ?></button>
                            <input type="submit" name="submit_aliases" class="button button-primary"
                                value="<?php _e('Save All Aliases', 'mmpro-email-aliases'); ?>">
                        </div>
                    </form>
                </div>
                ...
                <div class="mmpro-sidebar-column">
                    ...
                    <div class="mmpro-card">
                        ...
                        <div class="mmpro-export-buttons" style="margin-bottom: 15px;">
                            <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mmpro-email-aliases&action=export&format=json'), 'mmpro_export'); ?>"
                                class="button"><?php _e('Export as JSON', 'mmpro-email-aliases'); ?></a>
                            <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mmpro-email-aliases&action=export&format=csv'), 'mmpro_export'); ?>"
                                class="button"><?php _e('Export as CSV', 'mmpro-email-aliases'); ?></a>
                        </div>
                        ...
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <p>
                                <input type="submit" name="import_aliases" class="button button-primary"
                                    value="<?php _e('Import', 'mmpro-email-aliases'); ?>">
                            </p>
                        </form>
                    </div>
                    ...
                    <div class="mmpro-card">
                        ...
                        if ($registration_time) {
                            ...
                            echo sprintf(
                                __('REST API registered at: %s', 'mmpro-email-aliases'),
                                date('Y-m-d H:i:s', $registration_time)
                            ) . '</p>';
                        } else {
                        ...
                        <p>
                            ...
                            <?php if (!file_exists(ABSPATH . 'direct-api/aliases.php')): ?>
                                <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mmpro-email-aliases&action=direct_access'), 'mmpro_direct_access'); ?>"
                                    class="button">
    ...
    /**
     * Handle file exports properly (JSON and CSV)
     */
    ...
    {
        // Skip check entirely if debugging
        ...
        // If we have data, there's no immediate issue
        if ($current_count > 0) {
            return false;
        }
        ...
        // Only look at the 5 most recent entries
        $recent_logs = array_slice($debug_log, -5);
        ...
        // If we have saves with data followed by multiple page loads with no data,
        // that's a persistence issue
        return ($has_data_operations && $recent_zero_loads >= 2);
    }
    ...
    public function render_diagnostics_page()
    {
        ...
        <div class="wrap">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($backup_count > 0): ?>
                                <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mmpro-email-aliases-diagnostics&action=restore_backup'), 'mmpro_restore_backup'); ?>"
            ...
            <div class="mmpro-card">
                ...
                <table class="mmpro-status-table">
                    ...
                    <tr>
                        ...
                        <td><a href="<?php echo esc_url(rest_url()); ?>" target="_blank"><?php echo esc_html(rest_url()); ?></a>
                        </td>
                    </tr>
                    <tr>
                        <td><?php _e('Plugin Endpoint (Standard)', 'mmpro-email-aliases'); ?></td>
                        <td><a href="<?php echo esc_url(rest_url('mmpro/v1/aliases')); ?>"
                                target="_blank"><?php echo esc_html(rest_url('mmpro/v1/aliases')); ?></a></td>
                    </tr>
                    <tr>
                        <td><?php _e('Plugin Endpoint (Pretty)', 'mmpro-email-aliases'); ?></td>
                        <td><a href="<?php echo esc_url(home_url('api/aliases/')); ?>"
                                target="_blank"><?php echo esc_html(home_url('api/aliases/')); ?></a></td>
                    </tr>
                    <tr>
                        <td><?php _e('Test Endpoint', 'mmpro-email-aliases'); ?></td>
                        <td><a href="<?php echo esc_url(rest_url('mmpro/v1/test')); ?>"
                                target="_blank"><?php echo esc_html(rest_url('mmpro/v1/test')); ?></a></td>
                    </tr>
                    <tr>
                        <td><?php _e('Direct API Endpoint', 'mmpro-email-aliases'); ?></td>
                        <td>
                            <?php if (file_exists(ABSPATH . 'direct-api/aliases.php')): ?>
                                <a href="<?php echo esc_url(home_url('direct-api/aliases.php')); ?>"
                                    target="_blank"><?php echo esc_html(home_url('direct-api/aliases.php')); ?></a>
                            <?php else: ?>
            ...
            <div class="mmpro-card">
                ...
                <p>
                    <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mmpro-email-aliases-diagnostics&action=manual_register'), 'mmpro_manual_register'); ?>"
                        class="button"><?php _e('Manually Register API Endpoints', 'mmpro-email-aliases'); ?></a>
                    <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mmpro-email-aliases-diagnostics&action=flush_rules'), 'mmpro_flush_rules'); ?>"
                        class="button"><?php _e('Flush Rewrite Rules', 'mmpro-email-aliases'); ?></a>
                    <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mmpro-email-aliases-diagnostics&action=direct_api_test'), 'mmpro_direct_api_test'); ?>"
                        class="button"><?php _e('Test Direct API Access', 'mmpro-email-aliases'); ?></a>
                    <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mmpro-email-aliases-diagnostics&action=setup_direct_api'), 'mmpro_setup_direct_api'); ?>"
                        class="button button-primary"><?php _e('Set Up Direct API Access', 'mmpro-email-aliases'); ?></a>
                </p>
                ...
                <ol>
                    ...
                    <li><?php _e('Click "Set Up Direct API Access" to create a direct PHP endpoint', 'mmpro-email-aliases'); ?>
                    </li>
                    <li><?php _e('Update your Cloudflare worker to use the direct API instead of REST API', 'mmpro-email-aliases'); ?>
                    </li>
                </ol>

                <h3><?php _e('Alternative Cloudflare Worker Code', 'mmpro-email-aliases'); ?></h3>
                <p><?php _e('If you\'re experiencing data persistence issues, use this worker code that tries multiple endpoints:', 'mmpro-email-aliases'); ?>
                </p>
            ...
            <div class="mmpro-card">
                ...
                <table class="mmpro-status-table">
                    ...
                    <tr>
                        ...
                        <td><?php echo wp_using_ext_object_cache() ? __('External Object Cache Enabled', 'mmpro-email-aliases') : __('Using Database Options', 'mmpro-email-aliases'); ?>
                        </td>
                    </tr>
                    <tr>
        ...
        <div class="mmpro-alias-card" data-index="<?php echo esc_attr($index); ?>">
            ...
            <div class="mmpro-alias-content">
                <div class="mmpro-alias-field">
                    <label
                        for="alias-address-<?php echo esc_attr($index); ?>"><?php _e('Alias Email Address', 'mmpro-email-aliases'); ?></label>
                    <input type="email" name="aliases[<?php echo esc_attr($index); ?>][address]"
                        id="alias-address-<?php echo esc_attr($index); ?>" value="<?php echo esc_attr($alias); ?>"
                ...
                <div class="mmpro-destinations">
                    ...
                    <div class="mmpro-destinations-container">
                        ...
                        foreach ($destinations as $dest) {
                            ?>
                            <div class="mmpro-destination-item">
                                <input type="email" name="aliases[<?php echo esc_attr($index); ?>][destinations][]"
                                    value="<?php echo esc_attr($dest); ?>" class="regular-text" required>
                                <button type="button"
                                    class="button button-small mmpro-remove-destination"><?php _e('Remove', 'mmpro-email-aliases'); ?></button>
                            </div>
                            <?php
                        }
                        ?>
                    ...
                    <button type="button"
                        class="button mmpro-add-destination"><?php _e('Add Destination', 'mmpro-email-aliases'); ?></button>
                </div>
            </div>
        </div>
        <?php
